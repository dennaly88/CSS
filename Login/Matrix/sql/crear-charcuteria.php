...
$accion =$_POST['accion'];
                  ...
                  $query8 = "SELECT * from charcuteria where '$nombre'=nombre";
                  $result8 = pg_query($query8) or die('PROBLEMAS CON LA CONEXION: ' . pg_last_error());
                   if(pg_num_rows($result8)>0)
                   {
                        
                              header("Location:../principal-charcuteria.php?usu=6");  
                      }
        
                   else{
                   	           
                    $query = "INSERT INTO charcuteria(nombre,fecha,unidad,usuario,cantidad,accion) VALUES ('$nombre','$fecha','$unidad','$usuario','$cantidad','$accion')";
                    ...
                    $query2 = "INSERT INTO inv_charcuteria(nombre,fecha,unidad,usuario,cantidad,accion) VALUES ('$nombre','$fecha','$unidad','$usuario','$cantidad','$accion')";
                    
                    $result2 = pg_query($conexion, $query2) or die('ERROR AL INSERTAR DATOS: ' . pg_last_error());
                    $cmdtuples2 = pg_affected_rows($result2);
                    
                    header("Location:../principal-charcuteria.php?usu=1");
